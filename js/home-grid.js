...
(function(){
  ...
  function start(list){
    var g=gridEl(); if(!g) return;
    attachImageFallback(g);
    var s=sentinelEl(g);
    var i=0, PAGE=12, loading=false, done=false;
    function renderMore(){
  ...
  function normalize(list){
    for(var i=0;i<list.length;i++){
      ...
      if(it.gallery&&it.gallery.length) for(var j=0;j<it.gallery.length;j++){ var s=it.gallery[j]; it.gallery[j]=(s.charAt(0)==='/'?s:'/'+s.replace(/^\/+/,'')); }
    }
    return list;
  }
  function init(){
    loadList(function(list){
      start(normalize(Array.isArray(list)?list:[]));
    });
  }
