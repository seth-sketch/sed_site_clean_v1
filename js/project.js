...
(function () {
  ...
  function getSlug(){
    var s=null; if (location.search.indexOf("slug=")!==-1){
  ...
  function findThumb(el){
    while (el && el !== document.body) { if (el.classList && el.classList.contains("thumb")) return el; el = el.parentNode; }
    return null;
  }
  function normalize(list){
    for (var i=0;i<list.length;i++){
      ...
      if (it.gallery && it.gallery.length){
        for (var j2=0;j2<it.gallery.length;j2++){
          var s=it.gallery[j2]; it.gallery[j2]=(s.charAt(0)==="/" ? s : "/"+s.replace(/^\/+/,""));
        }
      }
    }
    return list;
