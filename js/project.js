...
(function () {
  ...
  loadWorkJSON().then(function (list) {
    ...
    // Optional per-project press link: item.press
    if (item.press) {
      var pWrap = document.getElementById('projectPress');
      var pA = document.getElementById('projectPressLink');
      if (pWrap && pA){
        pA.href = item.press;
    ...
    if (nextEl) {
      ...
      nextEl.setAttribute('href', '/project?slug=' + encodeURIComponent(nxt.slug));
