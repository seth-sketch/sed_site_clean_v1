(function () {
    ...
    for(var i=0;i<list.length;i++){
      ...
      if(it.gallery && it.gallery.length){
        for(var g=0; g<it.gallery.length; g++){
          var p=it.gallery[g]; it.gallery[g]=(p.charAt(0)==="/" ? p : "/"+p.replace(/^\/+/,""));
        }
      }
    }
    return list;
  ...
  function render(item, idx, list){
    ...
    var first = item && (item.cover || (item.gallery && item.gallery[0]));
    if(hero) hero.innerHTML = ratio(first);

    if(thumbs && item && item.gallery && item.gallery.length){
      var html="";
      for(var g=0; g<item.gallery.length; g++){
        var src=item.gallery[g];
        html+='<button class="thumb" data-src="'+src+'"><img loading="lazy" src="'+src+'" alt=""></button>';
      }
