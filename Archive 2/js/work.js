(function(){
  ...
  function h(t,a={},...k){
    ...
    Object.entries(a).forEach(([kk,v])=>{
      if(kk==='class') e.className=v;
      else if(kk==='html') e.innerHTML=v;
      else e.setAttribute(kk,v);
    });
    k.flat().forEach(c=>{
      if(c==null)return;
  ...
  function fetchJSON(u){
    return fetch(u+(u.includes('?')?'&':'?')+'v='+Date.now(),{cache:'no-store'}).then(r=>r.json());
  }
  function fixPath(p){
    if(!p) return p;
  ...
  function normalizeItem(raw, idx){
    ...
    const media   = (raw.media?.length)
      ? raw.media.map(m=>m&&m.type?{...m,src:fixPath(m.src)}:{type:'image',src:fixPath(m)})
      : gallery.map(src=>({type:'image',src:fixPath(src)}));

    const draftings = Array.isArray(raw.draftings)
      ? raw.draftings.map(fixPath)
      : (raw.draftings ? [fixPath(raw.draftings)] : []);
    const pdf = typeof raw.pdf==='string' && raw.pdf.trim()? fixPath(raw.pdf): null;
  ...
  const ctn=byId('work-lb-content'),
        strip=byId('work-lb-strip'),
        btnX=byId('work-lb-close'),
        btnP=byId('work-lb-prev'),
        btnN=byId('work-lb-next');
  ...
  function renderLB(){
    ...
    strip.innerHTML='';
    media.forEach((it,i)=>{
      const thumbSrc=it.type==='image'?it.src:'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="80"><rect width="120" height="80" fill="%23000"/><polygon points="45,25 45,55 75,40" fill="%23fff"/></svg>';
  ...
  function renderCard(item,openLightbox){
    ...
    // --- add PDF / drafting links under description ---
    const pdfLinks=[];
    if(item.pdf) pdfLinks.push(item.pdf);
    if(Array.isArray(item.draftings)&&item.draftings.length) pdfLinks.push(...item.draftings);

    if(pdfLinks.length){
      const linkWrap=h('div',{class:'pdf-links'});
      pdfLinks.forEach((href,i)=>{
        const a=h('a',{href, target:'_blank', rel:'noopener', class:'pdf-link'},
          pdfLinks.length>1?`View PDF ${i+1}`:'View PDF');
